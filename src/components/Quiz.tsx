...
export default function Quiz({musicData, buttonClickFinish, scoreState, setScoreState}:
                              {musicData: MusicData[], scoreState: number, buttonClickFinish: () => void, setScoreState: (state: (prevState: number) => number) => void}) {
  ...
  const nextButton = <button className={`${answerState ? 'block' : 'hidden'} btn-next`} onClick={handleNextButtonClick}>NEXT</button>
  ...
  const quizElements = (
    <>
      ...
      <div className="quiz-content">
          <div className="quiz-lyrics">Lyrics: </div><div className="lyrics-current">{currentQuestion.lyrics}</div>
          <div className="quiz-answers">Answers: </div><div className="answers-variants">{allQuestions.length ? currentQuestion.answers.map((answer: Answer) => {
